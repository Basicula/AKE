...
namespace Equations
  ...
  double CubicRoot(double i_val)
    {
    return i_val > 0.0 ? pow(i_val, 1.0 / 3) : (i_val < 0.0 ? -pow(-i_val, 1.0 / 3) : 0);
    }

  bool IsCloseToValue(double i_val, double i_expected)
    {
    return abs(i_val - i_expected) < eps;
    }

  int SolveQuadratic(const double* i_coefs, double *io_roots)
    {
    double common_solution = -i_coefs[1] / i_coefs[2];
    if (i_coefs[0] == 0)
      {
    ...
    double square_descriminant = i_coefs[1] * i_coefs[1] - 4 * i_coefs[0] * i_coefs[2];
    if (square_descriminant < 0)
      return 0;

    if (square_descriminant == 0)
      {
      io_roots[0] = common_solution / 2;
      return 1;
      }

    double descriminant = sqrt(square_descriminant);
    io_roots[0] = (descriminant - i_coefs[1]) / (2 * i_coefs[2]);
    io_roots[1] = -(descriminant + i_coefs[1]) / (2 * i_coefs[2]);
    return 2;
    }
  ...
  int SolveCubic(const double* i_coefs, double *io_roots)
    ...
    const double q = C + 2 * A * A * A / 27 - A * B / 3;
    ...
    now let x = u + v =>
            ...
            p = -3*u*v , q = -(u^3 + v^3) =>
            u^3 * v^3 = -p^3/27
    we have simple representation for u^3 and v^3 like roots for quadratic equation as x^2 - (x1+x2)*x + x1*x2 = 0 where x1,x2 are roots of this equation
    so from x^2 + qx - p^3/27 = 0 have D = q^2 + 4*p^3/27 or D = 27*q^2 + 4*p^3 or D = q^2 / 4 + p^3/27
    */
    ...
    */
    if (IsCloseToValue(D, 0.0))
      {
      if (IsCloseToValue(q, 0.0))
        {
        io_roots[0] = 0;
        roots_count = 1;
        }
      ...
      {
      io_roots[0] = CubicRoot(-q / 2 + sqrt(D)) + CubicRoot(-q / 2 - sqrt(D));
      roots_count = 1;
      }
    else
      {
      const double PI = acos(-1);
      const double angle = acos(3 * q * sqrt(-3 / p) / (2 * p)) / 3;
      const double t = 2 * sqrt(-p / 3);
      io_roots[0] = t * cos(angle);
      io_roots[1] = t * cos(angle - 2 * PI / 3);
      io_roots[2] = t * cos(angle + 2 * PI / 3);
      roots_count = 3;
      }
  ...
  int SolveQuartic(const double* i_coefs, double *io_roots)
    ...
    const double p = B - 3 * square_A / 8;
    const double q = C - A * B / 2 + square_A * A / 8;
    const double r = D - A * C / 4 + square_A * B / 16 - 3 * square_A * square_A / 256;

    int roots_count = 0;
    if (IsCloseToValue(r, 0.0))
      {
      /*
      ...
      x(x^3 + px + q) = 0
      */
      const double cubic_coefs[4] = { q,p,0,1 };
      roots_count = SolveCubic(cubic_coefs, io_roots);
      io_roots[roots_count++] = 0;
      }
    else
      {
      /*
      ...
      */
      const double cubic_coefs[4] = { -q * q, 2 * p*p - 8 * r,8 * p,8 };
      ...
      double sqrt_double_cubic_res = sqrt(2 * cubic_res);
      const double quadratic_coefs1[3] = { p / 2 + cubic_res - q / (2 * sqrt_double_cubic_res), sqrt_double_cubic_res, 1 };
      roots_count = SolveQuadratic(quadratic_coefs1, io_roots);
      const double quadratic_coefs2[3] = { p / 2 + cubic_res + q / (2 * sqrt_double_cubic_res), -sqrt_double_cubic_res, 1 };
