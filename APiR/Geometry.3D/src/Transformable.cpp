...
void Transformable::Scale(const Vector3d& i_factors)
{
  ...
  m_transformation.SetScale(
    Vector3d(old_scale[0] * i_factors[0], old_scale[1] * i_factors[1], old_scale[2] * i_factors[2]));
  _OnTransformationChange();
}
...
Vector3d Transformable::ApplyScaling(const Vector3d& i_point) const
{
  ...
  return Vector3d(scaling[0] * i_point[0], scaling[1] * i_point[1], scaling[2] * i_point[2]);
}
...
Vector3d Transformable::ApplyRotation(const Vector3d& i_point) const
{
  ...
  return rotation.ApplyLeft(i_point);
}
...
{
  return Ray(m_transformation.InverseTransform(i_ray.GetOrigin()),
