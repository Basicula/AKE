...
namespace Equations
  ...
  double CubicRoot(double i_val)
    {
    return i_val > 0.0 ? std::pow(i_val, 1.0 / 3) : (i_val < 0.0 ? -std::pow(-i_val, 1.0 / 3) : 0);
    }
  ...
  int SolveQuadratic(const double* i_coefs, double *io_roots)
    ...
    const double square_descriminant = i_coefs[1] * i_coefs[1] - 4 * i_coefs[0] * i_coefs[2];
  ...
  int SolveCubic(const double* i_coefs, double *io_roots)
    ...
    */
    if (D == 0.0)
      ...
      {
      io_roots[0] = CubicRoot(-q / 2 + std::sqrt(D)) + CubicRoot(-q / 2 - std::sqrt(D));
      roots_count = 1;
      }
    else
      {
      const double PI = std::acos(-1);
      const double angle = std::acos(3 * q * std::sqrt(-3 / p) / (2 * p)) / 3;
      const double t = 2 * std::sqrt(-p / 3);
      io_roots[0] = t * std::cos(angle);
      io_roots[1] = t * std::cos(angle - 2 * PI / 3);
      io_roots[2] = t * std::cos(angle + 2 * PI / 3);
      roots_count = 3;
      }
  ...
  int SolveQuartic(const double* i_coefs, double *io_roots)
    ...
    if (r == 0.0)
      ...
      */
      ...
      double sqrt_double_cubic_res = std::sqrt(2 * cubic_res);
