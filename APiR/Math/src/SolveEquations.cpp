...
namespace Equations
  ...
  double CubicRoot(double i_val)
    {
    return i_val > 0.0 ? 
      std::pow(i_val, 1.0 / 3.0) : 
      (i_val < 0.0 ? -std::pow(-i_val, 1.0 / 3.0) : 0.0);
    }
  ...
  int SolveQuartic(const double* i_coefs, double *io_roots)
    ...
    if (r == 0.0)
      ...
      */
      ...
      const double quadratic_coefs1[3] = { p / 2.0 + cubic_res - q / (2.0 * sqrt_double_cubic_res), sqrt_double_cubic_res, 1.0 };
      roots_count = SolveQuadratic(quadratic_coefs1, io_roots);
      const double quadratic_coefs2[3] = { p / 2.0 + cubic_res + q / (2.0 * sqrt_double_cubic_res), -sqrt_double_cubic_res, 1.0 };
