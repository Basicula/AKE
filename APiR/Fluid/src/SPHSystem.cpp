...
void SPHSystem::BuildNeighborsList()
{
  ...
  ThreadPool::GetInstance()->ParallelFor(static_cast<std::size_t>(0), m_num_particles, [&](std::size_t i) {
    const Vector3d& origin = points[i];
    m_neighbors_list[i].clear();

    mp_neighbor_searcher->ForEachNearbyPoint(origin, m_radius, [&](std::size_t j, const Vector3d&) {
      if (i != j) {
