...
struct Mandelbrot
  {
  std::size_t m_width = 1024;
  std::size_t m_height = 768;
  int m_origin_x = 0;
  int m_origin_y = 0;
  std::size_t m_max_iterations = 256;
  int m_scale = 1;
  const std::uint8_t m_color_map[17 * 3] =
    {
    0, 0, 0,
    66, 45, 15,
    25, 7, 25,
...
class OpenCLKernel
  ...
  public:
    ...
    static const std::size_t mg_max_info_size = 256 * 256;
  ...
  public:
    ...
    void MandelbrotSetInit(std::size_t i_width, std::size_t i_height, std::size_t i_max_iterations = 1000);
