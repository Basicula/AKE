...
__global__ void mandelbrot_set_kernel(Image* iop_image,
                                      const MandelbrotSet* ip_fractal,
                                      const custom_vector<Color>* ip_colors)
{
  int x = threadIdx.x;
  int y = blockIdx.x;
...
void test_cuda()
{
  ...
  device_vector_ptr<Color> d_colors = custom_vector<Color>::device_vector_ptr({ Color(0, 0, 0),
                                                                                Color(66, 45, 15),
                                                                                Color(25, 7, 25),
                                                                                Color(10, 0, 45),
                                                                                Color(5, 5, 73),
  ...
  auto update_func = [&]() { mandelbrot_set_kernel.Run(image.get(), mandelbrot_set.get(), d_colors.get()); };
  GLUTWindow window(width, height, "CudaTest");
  window.InitWindowBackend<ImageWindowBackend>(image.get());
